...
%     [ r_hat,x_hat,P_hat ] = mixture_reduction( phi,h_r,h_x,h_p,model );
    [rr_hat,xx_hat,PP_hat,qa,temp] = expectation(w,rr,xx,PP,r_hat,x_hat,P_hat);
    maxIteration = 10;
    iteration = 0;
    while(1)
        iteration = iteration + 1;
        [r_hat,x_hat,P_hat] = mixtureReduction2(w,qa,rr_hat,xx_hat,PP_hat);
        [qa,Cmin] = new_qa(w,r_hat,x_hat,P_hat,rr_hat,xx_hat,PP_hat);
        if temp-Cmin < 0.1 || iteration == maxIteration
            break;
        else
            temp = Cmin;
        end
...
%         if Cmin < temp && temp - Cmin < 0.1 || iteration == maxIteration
%             break;
%         else
%             temp = Cmin;
%         end
